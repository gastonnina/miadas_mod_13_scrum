...
- `create_temporal_split.py`
  - Crea particiones `dev`, `backtest` y `holdout_3m` por rango temporal desde `data/raw`.
  - Salida en `data/splits/temporal_2018q4/`.
  - Genera también `backtest_ids.parquet`, `holdout_3m_ids.parquet` y `holdout_3m_metadata.json`.

3. Pipeline Sprint 2/Sprint 4 completo (dev + backtest) — un solo comando
- `build_features.sh`
  - Orquesta los 7 pasos en orden: master table dev → features dev → selección experimental → evaluación vs baseline → master table backtest → features backtest → alineación final para scoring del MVP.
  ...
  # [5/7] Master table BACKTEST — aplica umbral fijo (apply), no recalcula
  $CONDA src/data/build_master_table.py --profile-source backtest --threshold-mode apply

  # [6/7] Features RFM BACKTEST — backtest oficial de agosto 2018
  $CONDA src/features/build_rfm_features.py \
  ...
  $CONDA scripts/build_holdout_scoring_dataset.py --profile-name backtest
  ```
  ...
  | `data/processed/03_master_table_clean_backtest.parquet` | Master table limpia (backtest) |
  | `data/processed/backtest_features_rfm.parquet` | Features RFM backtest |
  | `data/processed/backtest_features_selected.parquet` | Dataset final de scoring alineado a `modelo_final.pkl` |
