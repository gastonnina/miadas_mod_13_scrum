...
\midrule
Master table & \texttt{build\_master\_table.py} & \texttt{\{perfil\}/*.parquet} &
  \texttt{01\_raw\_\{perfil\}.parquet}, \texttt{03\_clean\_\{perfil\}.parquet}, \texttt{threshold.json} \\
Features & \texttt{build\_rfm\_features.py} & \texttt{03\_clean\_\{perfil\}.parquet} &
...
\begin{itemize}
    ...
    \item Los artefactos de la master table usan un prefijo de perfil en el nombre (\texttt{\_dev}, \texttt{\_holdout}, \texttt{\_raw}) para evitar colisiones entre ejecuciones sobre distintos conjuntos de datos. El script orquestador \texttt{scripts/build\_features.sh} corre el pipeline completo: pasos 1--4 sobre el split \textit{dev} y pasos 5--6 sobre el \textit{holdout} con umbral en modo \texttt{apply}.
\end{itemize}
...
\section{Validación Temporal con Holdout}

El pipeline incluye un ciclo de simulación mensual sobre el periodo de holdout (agosto--octubre 2018, ventana \texttt{temporal\_2018q4}), ejecutado con el umbral fijo calculado sobre \textit{dev} mediante \texttt{--threshold-mode apply}. El objetivo es verificar que el umbral y las features generalizan a datos no vistos antes de pasar al Sprint 3.

La Tabla~\ref{tab:holdout} compara los KPIs principales entre el conjunto \textit{dev} y el periodo de holdout.

\begin{table}[h]
\centering
\caption{Comparación KPIs: DEV vs Holdout (ago--oct 2018)}
\label{tab:holdout}
\begin{tabular}{lrr}
\toprule
KPI & DEV & Holdout \\
\midrule
Clientes activos       & 96.096 & 6.468 \\
Clientes premium       & 19.220 (20.00\%) & 1.253 (19.37\%) \\
Umbral aplicado        & BRL 197.01 (\texttt{fit}) & BRL 197.01 (\texttt{apply}) \\
Ticket prem. / reg.    & 4.36× & 4.47× \\
\bottomrule
\end{tabular}
\end{table}

La tasa premium del holdout (19.37\%) y el ratio de ticket (4.47×) son consistentes con los valores del conjunto \textit{dev}, lo que confirma dos propiedades del pipeline: (1) el umbral fijo no introduce sesgo temporal al aplicarse sobre una población distinta, y (2) las features del Sprint 2 capturan patrones de comportamiento estables entre periodos. La convergencia de ambas métricas valida la elección del P80 \textit{dev} como referencia permanente para los sprints siguientes.
...
\begin{enumerate}
    ...
    \item \textbf{Validación con holdout}: simulación sobre agosto--octubre 2018 confirma tasa premium de 19.37\% y ratio de ticket de 4.47×, coherentes con \textit{dev}, validando la estabilidad del umbral fijo y las features ante datos no vistos.
\end{enumerate}
