...
\subsection{Formulación de Hipótesis Iniciales}
Con el fin de guiar la exploración analítica y el modelado predictivo, se definieron cinco hipótesis de negocio iniciales que contrastan el comportamiento esperado del cliente premium frente a la base general:
\begin{itemize}
    \item \textbf{H1 (Frecuencia):} Los clientes con mayor frecuencia de compra acumulada en la plataforma tienen una probabilidad significativamente mayor de ser catalogados como Premium.
    \item \textbf{H2 (Calificaciones):} Los clientes con puntuaciones promedio de reseñas más altas muestran una mayor afinidad con la marca y tienden a pertenecer al segmento Premium.
    \item \textbf{H3 (Recencia):} Los clientes con menor recencia temporal (compras más recientes) poseen una mayor probabilidad de ser Premium.
    \item \textbf{H4 (Monetario):} Los clientes del segmento Premium presentan tickets promedio unitarios significativamente superiores en comparación con los clientes normales.
    \item \textbf{H5 (Poder Predictivo RFMT):} Las variables transaccionales de comportamiento temporal y de servicio (RFMT) son suficientes por sí solas para entrenar un clasificador discriminante efectivo de clientes Premium.
\end{itemize}
...
La Figura~\ref{fig:long_tail} ilustra la distribución del gasto acumulado por cliente, donde se aprecia la marcada cola derecha y la posición del umbral del Percentil 80 (\$205.18 USD) que define la variable objetivo.

\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{fig_long_tail.png}
  \caption{Distribución del gasto acumulado neto por cliente. La línea discontinua indica el umbral del Percentil 80 (\$205.18 USD), adoptado como corte para \textit{is\_premium} dado el sesgo extremo de la distribución (skewness $= 7.92$).}
  \label{fig:long_tail}
\end{figure}
...
\subsection{Curva ROC del Modelo Baseline}
La Figura~\ref{fig:roc} presenta la curva ROC del modelo de regresión logística entrenado. Se puede observar que la curva se aleja apenas del clasificador aleatorio, coherente con el valor de $\text{ROC-AUC} = 0.5355$ reportado en la Tabla~\ref{tab:metrics}.

\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{fig_roc_baseline.png}
  \caption{Curva ROC del modelo baseline (regresión logística con clase balanceada). El área bajo la curva ($\text{AUC} = 0.5355$) es marginalmente superior al clasificador aleatorio, evidenciando la insuficiencia de las variables RFMT tradicionales para discriminar el segmento Premium.}
  \label{fig:roc}
\end{figure}

\subsection{La Paradoja del Target}
El modelo baseline técnico presenta un rendimiento muy modesto ($ROC-AUC = 0.5355$), apenas superior a una asignación aleatoria. Este resultado confirma empíricamente la \textbf{Paradoja del Target} y contradice la hipótesis de predictibilidad básica \textbf{H5}: el comportamiento de compra de clientes Premium en Olist está determinado por una \textbf{compra única de alto valor} y no por una conducta de alta frecuencia y lealtad temporal. Las variables basadas estrictamente en la frecuencia de pedidos (\texttt{total\_orders}) o el lifetime de relación (\texttt{customer\_lifetime\_days}) son idénticas para el $97\%$ de los clientes (con solo 1 orden), impidiendo la diferenciación estadística usando únicamente RFM tradicional (contradiciendo la hipótesis \textbf{H1}).

\subsection{Métricas y Relaciones de Negocio}
El análisis descriptivo de segmentos revela valiosas relaciones comerciales que contrastan nuestras hipótesis iniciales:
\begin{itemize}
    \item \textbf{Ticket Medio (Validación H4):} El ticket promedio del cliente Regular es de $93.99\$$ USD, mientras que en el segmento Premium alcanza \textbf{\$416.14 USD} ($4.4$ veces superior). Esto valida plenamente la hipótesis \textbf{H4}, confirmando la concentración monetaria del segmento.
    \item \textbf{Satisfacción en Clientes Premium (Validación H2):} Las reseñas promedio de los clientes Premium son sutilmente \textbf{menores} ($4.04$ estrellas) frente a los clientes normales ($4.10$). Esto contradice la hipótesis de mayor satisfacción \textbf{H2}, revelando que los clientes de alto ticket poseen expectativas superiores y menor tolerancia ante fallas.
    \item \textbf{Retrasos Logísticos (Validación H3):} El porcentaje de entregas con retraso (\texttt{late\_delivery\_rate}) es del \textbf{8.87\%} en el grupo Premium frente al $7.67\%$ en clientes regulares. Esto contradice la hipótesis logística de priorización \textbf{H3}, lo que corrobora que las compras caras involucran productos de mayor tamaño y complejidad de distribución, elevando los retrasos.
\end{itemize}
